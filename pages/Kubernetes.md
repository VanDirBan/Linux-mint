...
- **Running a Free Kubernetes Cluster Online – Play with Kubernetes (PWK)**
	- **Definition**:
		- Play with Kubernetes (PWK) is an online Kubernetes sandbox that allows users to create and test Kubernetes clusters in a browser for free.
		- It provides temporary clusters for learning and experimenting with Kubernetes concepts.
	- **Accessing Play with Kubernetes**
		- **Website**: [https://labs.play-with-k8s.com](https://labs.play-with-k8s.com)
		- **Requirements**:
			- A Docker Hub account is required to log in.
			- No local installation is needed.
		- **Limitations**:
			- Clusters are available for a limited time (up to 4 hours).
			- Not intended for production use.
	- **Creating a Free Kubernetes Cluster**
		- **Steps**:
		  1. **Login with Docker Hub**:
			- Visit [PWK](https://labs.play-with-k8s.com) and sign in with Docker credentials.
			  2. **Start a New Session**:
			- Click "Start" to begin a new interactive environment.
			...
			- If multiple nodes are needed:
			  ...
			  ```
			  4. **Configure kubectl**:
			- Run the following to set up access:
			  ```bash
			  mkdir -p $HOME/.kube
	...
	- **Deploying an Application**
		...
		- **Get Service URL**:
		  ...
		  kubectl get services
		  ```
	- **Best Practices for Using Play with Kubernetes**
		- Save important configurations externally before the session expires.
		- Use multiple nodes to simulate real cluster environments.
		- Practice Kubernetes commands and test workloads in a controlled sandbox.
		- Understand that all data is lost when the session ends.